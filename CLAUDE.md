...
- **Catalog** (source of truth): the released `github.com/hollis-labs/go-envelopes` module selected by `go.mod`. Its public catalog/exporter supplies the canonical manifest, schemas, annotations, generated data types, and import metadata without locating module-cache files or a sibling checkout.
- **Schema**: owned, compiled, and exposed through `go-envelopes`' `SchemaDocument` / `SchemaMetadata` APIs; Nanite interprets only host-specific annotations such as `default_render_target`
...
- **Frontend** (TypeScript): `ui/src/generated/envelope-types.generated.ts` comes from the module-owned TypeScript generator; `ui/src/generated/plugin-envelopes.ts` consumes the exported catalog and adds only Nanite's React loader plus host-side `CORE_OVERRIDES`
- **Plugin types**: declared in `plugins/*/plugin.yaml` under `registers.envelopes`

**When adding a new core envelope type:**
1. Release the entry and schema in `go-envelopes`, or — for a host-only frontend mapping — add a `CORE_OVERRIDES` entry in `scripts/generate-plugin-imports.mjs`
2. Create the React component in `ui/src/components/chat/envelopes/`
3. Run `make generate-envelopes && npm run generate:plugins` (both also run automatically on frontend build/dev)
...
**Staleness check — manual, not enforced by a hook.** `make check-envelopes` verifies both generated artifacts against the exact module-selected exporter output. Frontend `prebuild`/`predev` regenerates them so `npm run build` and `npm run dev` stay clean-clone safe.
