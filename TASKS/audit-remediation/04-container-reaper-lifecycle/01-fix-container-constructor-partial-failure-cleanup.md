...
> **Wave 0 revalidation (2026-08-22).** The line numbers below were already once revalidated (per this section's own header) against an earlier HEAD, but further drift landed since — `internal/store` changes are unrelated to this file, but two commits added fields (`SkillVendor`, `ReflexEngine`) earlier in `container.go` and shifted everything below by +51 to +53 lines. Corrected to the current, re-verified line numbers; the code itself (this function's logic) is byte-for-byte unchanged from the audited commit `8feeee5c`.

- `reaperCtx, stopReaper := context.WithCancel(context.Background())` at `internal/service/container.go:1213` (was `1162`) immediately starts `subagentReaper.Start(reaperCtx)` — a background goroutine polling `subagent_runs`.
- `runtimeReaperCtx, stopRuntimeReaper := context.WithCancel(context.Background())` at `internal/service/container.go:1233` (was `1182`) immediately starts `runtimeReaper.Start(runtimeReaperCtx)` — a background goroutine polling `agent_runtime`.
- Both cancel funcs are captured into the `Container` struct only near the end of the constructor, at `internal/service/container.go:1451` (was `1398`; `stopSubagentReaper: stopReaper`) and `internal/service/container.go:1453` (was `1400`; `stopRuntimeReaper: stopRuntimeReaper`).
- Two confirmed early-return paths execute between reaper start and struct assembly without calling either cancel func (re-verified against current source):
  - `internal/service/container.go:1293` (was `1241`) — `if err != nil { stopCatalog(); return nil, fmt.Errorf("service container: durable agent recipes: %w", err) }`, guarding `NewDurableAgentRecipeService(...)` at line 1290 (was `1239`).
  - `internal/service/container.go:1297` (was `1245`) — `if err := SyncManagedDurableAgentConfigs(cfg.Store, managedConfigRoot); err != nil { stopCatalog(); return nil, fmt.Errorf("service container: sync managed durable agents: %w", err) }`.
...
- `go vet ./...` independently flags both cancel funcs (quoted verbatim from the audit-era run; line numbers in this specific quote reflect the audited commit and will shift to `1213`/`1233` on a fresh run against current `HEAD`, per the drift above): `internal/service/container.go:1162:2: the stopReaper function is not used on all paths (possible context leak)` and `internal/service/container.go:1182:2: the stopRuntimeReaper function is not used on all paths (possible context leak)`.
