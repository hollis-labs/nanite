...
| `internal/plugin/envelope_validator.go:81` | 1 | external unauthenticated | `install-local` / `install-archive` → hot-load manifest registration → lexical `resolvePluginAssetPath` → schema read. Archive extraction rejects links; local install has already dereferenced any source link at `plugins.go:969`. |
...
preserves a hostile symlink was established: archive extraction rejects links,
and local installation dereferences a source link at `plugins.go:969` and
