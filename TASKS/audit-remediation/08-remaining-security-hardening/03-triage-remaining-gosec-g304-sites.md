...
68 production gosec G304 (path-traversal-via-variable) findings exist outside the `pathsafe.ResolveUnder` forbidigo-tracked adoption scope — `.golangci.yml`'s `forbidigo` rules only enforce `ResolveUnder` adoption inside `internal/sandbox/`, `internal/mcp/`, `internal/service/install/`; none of these 68 sites are in that path list. This is real, tracked-but-incomplete-migration debt, not a fresh discovery — the project already has the `pathsafe.ResolveUnder` primitive and an active adoption tracker; the tracker's scope just doesn't cover these sites yet.
