...
| 09 | `09-autocomplete-and-artifact-path-hardening.md` | GO-API-001, GO-API-002, GO-API-003, GO-API-008, GO-SEC4-007 | Five `internal/api`/egress hardening findings: write-time project `repo_path` policy, artifact and plugin-UI path confinement, catalog-download SSRF destination blocking after the timeout/size half was already resolved, and extraction of the duplicated MCP/sandbox CIDR policy for all three consumers. |
