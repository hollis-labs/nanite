...
| **Prevention** | (a) `go vet` as a **blocking** gate (it is already in `lefthook.yml` pre-commit, but `GO-LIFE-001` still landed — worth `00/02` establishing whether the hook was bypassed or the finding predates it). (b) Fix `lint-goroutines` to cover all packages and actually fail (`12/03`). (c) A constructor-cleanup pattern documented once, with `stopCatalog()`'s existing correct usage in the same function as the in-repo example. |
| **Enforcement point** | `Makefile` `lint-goroutines` (`12/03`), `lefthook.yml` go-vet, `12/02` standard. |
