...
**2026-08-19 — follow-up closure (TASKS/phase-2/06-cut-nanite-native-adapter-agent-sync.md):** This task's explicit carve-out — keeping `nanite-native`'s `Discover` alive "because it's also how the nanite-native adapter's Discover... gets invoked, and that one is not being cut" — is now closed. Phase 2 task 06 cut both of `adapter-nanite-native`'s remaining `.nanite/config.yaml` -> `agent_profiles` write paths: `Plugin.Load()`'s direct `store.UpsertAgentBySlug` sync, and `Adapter.Discover()`'s parsing into the `AutoIngestAgents` pipeline via `internal/agent/discovery.go`'s adapter tier. Both are now no-ops (mirroring the precedent this task set for the four external-format adapters). `PopulateSandbox` (the different, DB/config -> disposable-sandbox-file direction this task also correctly identified as unrelated) was reconfirmed still live and unaffected. See that task file for full details.
