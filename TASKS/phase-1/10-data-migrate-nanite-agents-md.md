...
- The `.nanite/agents/*.md` files themselves are **not deleted** by this decision — they remain in place (backed-up copies exist separately) and continue to be discovered/ingested as `agent_profiles` rows exactly as today. **Correction, 2026-08-18 (after task `02` landed)**: this doesn't mean literally zero Phase 1 tasks ever touch these files. Task `02` mechanically updated `activationMode: instance` → `fresh-per-wake` in 3 files' frontmatter (`atlas-curator.md`, `content-writer.md`, `loom-curator.md`) — required because task `02` renamed that enum value's valid set at the Go layer, and the old literal would otherwise fail boot-time ingest validation. This is a pure string-literal rename to keep parsing working, not content migration or role/scope grouping — no agent's persona, prompt, or tool grants were touched. Genuinely no task migrates content into `roles`/`agents`, which is what this file's cut is actually about.
