...
**Phase:** 1
**Status:** not-started
**Depends on:** `04-add-known-tools-and-agent-tools-fk.md` (schema, live-sync, and one-time legacy-column backfill must exist first)
...
6. Update/extend `TASKS/phase-1/09-build-assignment-ui-api.md`'s picker UI expectations if this task's answer to (1)/(4) changes what "revoking a tool" or "an agent with zero explicit grants" means for an operator using that UI.
