...
**Invocation path (CW-20260814-0003).** `WorkflowDefinition.Engine` selects which registered `WorkflowEngine` runs a definition — `""`/`agentworkflow.EngineBuiltin` (default), `agentworkflow.EngineLangGraph`, or `agentworkflow.EngineCrewAI`. `WorkflowLauncher.Launch` resolves the name against the engine set its caller wired in and errors clearly if that engine isn't registered on this process (e.g. no Python on PATH). `service.ExternalWorkflowEngine` is the `WorkflowEngine` adapter over `internal/workflowrunner.Launch` for LangGraph/CrewAI: its `Run` ignores the `StepExecutor` parameter entirely — an external engine's callbacks reach the *same* `StepExecutor` instance indirectly, through the three MCP tools, which are wired to that instance at the composition root. The hand-authored runner scripts (originally the LangGraph/CrewAI POCs) live in `internal/workflowrunner/scripts/`, embedded into the binary via `go:embed` and materialized to the app's state dir at startup (`workflowrunner.MaterializeScripts`) — a Cerberus-deployed binary ships alone, with no repo checkout alongside it, so `examples/` was never a viable production location. There is still no compiler from Nanite's own step-DAG format to LangGraph's/CrewAI's native shapes: an external-engine-targeted `WorkflowDefinition.Steps` is not consumed by `ExternalWorkflowEngine.Run` — the DAG shape is whatever the runner script hand-authors.
