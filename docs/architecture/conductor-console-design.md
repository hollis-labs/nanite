...
|---|---|---|
| Deterministic phrase-router | Live, wired into dispatch | `internal/reflex/` (singular — matcher, dispatcher, catalog, loader) — feeds `dispatch.AssignRole` via `ReflexHints`. Similar name only vs. the unrelated FU-30 predicate/event/interval steering engine at `internal/agent/reflexes` (plural, under `internal/agent/`) — different system, different code, different lifecycle. (That engine was briefly named `internal/agent/driftguard` per `CW-20260816-0062`, then renamed back.) |
