...
|---|---|---|
| Deterministic phrase-router | Live, wired into dispatch | `internal/promptrouter/` (matcher, dispatcher, catalog, loader) — feeds `dispatch.AssignRole` via `ReflexHints`. Unrelated to the FU-30 predicate/event/interval steering engine at `internal/agent/reflexes` (under `internal/agent/`) — different system, different code, different lifecycle. (`internal/promptrouter` was previously named `internal/reflex`, renamed to remove the naming collision with `internal/agent/reflexes`. That engine was itself briefly named `internal/agent/driftguard` per `CW-20260816-0062`, then renamed back.) |
...
## Deterministic input middleware

Extend `internal/promptrouter/` with a rule set for the user's actual directive vocabulary: note-capture triggers ("note for Nil"), dispatch-to-project triggers, non-linear "research X, drop results in Y" triggers (routes to Torque task creation rather than synchronous dispatch), and approval phrasing. Start as user-level YAML overrides (`~/.nanite/reflexes/*.yaml`) for fast iteration; promote to built-ins (`internal/promptrouter/catalog.go`) later once the rule set stabilizes.

One genuine gap beyond existing infrastructure: the user wants both the **raw input** and the **actual rewritten/dispatched version** persisted together for later audit and improvement. The existing `playbook_match_log` captures match ID/profile/mode but not a raw-vs-rewritten text diff — this needs a small, scoped extension (see task T3).
