...
- **Concurrency teardown.** UnloadPlugin deadlock (plugin 01), worker shutdown races (already fixed but worth regression-guarding). Panic propagation out of `Host.EmitEvent` and elsewhere is now tracked under its own "No panic recovery" theme below, not here.
