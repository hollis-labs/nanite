...
These are genuinely distinct, not redundant — no consolidation of the mechanisms themselves is needed. **Done**: the four now live under a shared `internal/recovery/{broker,orphansweep,pack}/` namespace, matching the `agent/*`/`prompt/*` convention. **Partially done**: `event_log` postmortem logging, originally only written by the Recovery Broker, now also covers the Orphan/Runtime Reaper (`orphansweep.SweepOrphans`); Recovery Pack and interrupted-turn detection haven't been independently re-verified — treat as open, not as confirmed-missing.

See also [24-typed-corruption-recovery-taxonomy.md](24-typed-corruption-recovery-taxonomy.md) — a follow-up audit of the Recovery Broker's classification logic found it already mostly typed/enumerated (one heuristic substring-match survivor), and documents a target (not yet decided) taxonomy for the genuinely absent piece: conversation-state inconsistency detection (orphaned tool calls, sequence gaps), which doesn't exist anywhere in this recovery system today.
