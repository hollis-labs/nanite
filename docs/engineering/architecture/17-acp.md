...
A new `Adapter`/`RuntimeAdapter` implementation per underlying agent, in `go-agent-wrapper`'s `adapters/` package — the host [16-agent-host.md](16-agent-host.md) adopted and is now executing the migration onto — alongside `adapters/claude`/`adapters/codex`/`adapters/opencode`.
...
- **Which bridge(s) to pin** is left undecided for now — deliberately, given how young and fast-moving this ecosystem is (the field itself widened mid-review: single-vendor bridges turned up that didn't exist in the first pass). One real criterion worth weighing at implementation time: whether a given bridge actually wires ACP's `session/cancel` through to a real native interrupt, versus just acknowledging cancellation at the wire level and letting the turn finish anyway. **As of the implementation batch's own verification against the live code (`TASKS/agent-host-acp/README.md`), neither Claude's nor Codex's `Stop()` calls a native interrupt today — only OpenCode's does** (see the corrected limitation below). Re-check library state and behavior at implementation time rather than trusting this review's snapshot.

## Known limitations carried forward, not solved by adding ACP

- **Real mid-turn interrupt is provider-dependent, and — corrected against the actual implementation batch's own verification — currently real for exactly one provider, not two.** An earlier draft of this doc claimed Claude's Agent SDK (`Query.interrupt()`) and Codex's `app-server` (`turn/interrupt`) both have native interrupt mechanisms already wired through. That overstated it: those are real, documented surfaces on those SDKs, but **neither is actually called by `agentkit/agentsessions`, `go-agent-wrapper`, or Nanite's own current code today** — both do stdin-close + SIGTERM/SIGKILL only, at the process level, regardless of which ACP bridge is eventually chosen. **Only OpenCode's `Stop()` calls a real native abort endpoint** (`/global/dispose`, `/session/{id}/abort`) — confirmed directly in `TASKS/agent-host-acp/README.md`, the implementation batch's own fact-check pass against the live code. This is a carried-forward limitation, not a regression — Nanite's own `internal/service/agent_deps.go` already carries a TODO acknowledging the same gap independently. The genuine, narrower limitation this doc should carry forward is exactly what it was scoped to: **the host can only offer what the provider's own protocol exposes and what the plumbing between them actually calls, mediated by however faithfully the chosen bridge maps to it.** For Claude/Codex today, that's process-kill-level termination, not cooperative mid-generation interrupt — reliable and correct as a Stop, just not a true interrupt. For a provider with no native interrupt of its own (unverified for Pi at review time), or a bridge that doesn't wire an available native mechanism through, cancellation degrades the same way. This is a per-bridge, per-provider fact to re-verify at implementation time as SDKs and bridges evolve, not something to assume fixed either way. Tracked as the same open item in [16-agent-host.md](16-agent-host.md). Rather than treating every adapter's `Stop()`/cancel as identically capable, the descriptor should be able to say so — a small capability-discovery vocabulary (something like `interrupt: none | process | turn | steer`, exact naming TBD at implementation time) advertised per `Protocol`/`Transport` combination, so the host can accurately report "OpenCode: turn-level cancel" vs. "Claude/Codex: process-kill only" instead of presenting a uniform `Stop()` that quietly means different things underneath. Same principle as the rest of this doc: abstract the intent (cancel this turn), not the capability (pretending everything can).
- **Naming: ACP's `session/cancel` is semantically turn-scoped, despite its name — don't let the wire-protocol name dictate ours.** The ACP spec method is called `session/cancel`, but per the spec it cancels the in-flight *prompt turn*, not the session itself (the session survives and can accept the next prompt). This maps onto our own `Turn.Cancel` concept (see [19-api-cli-runtime-parity.md](19-api-cli-runtime-parity.md)), not onto `Session.Stop` (`internal/runtime/agent/manager.go`, which terminates the whole process). When the ACP client abstraction implements this method, it should call our `Turn.Cancel` path — never our `Session.Stop` path — regardless of what the wire method is named. Getting this backwards would mean an ACP client's cancel button kills the whole long-lived agent process instead of just the in-flight turn.
