...
**Correction (`TASKS/skills/01`'s review, 2026-08-21):** the claim above is accurate as of this doc's target state, but was not quite accurate as an audit of the pre-`TASKS/skills/01` code — a real, narrow invocation path existed and worked: the 8 embedded builtin skills (cut by task `01`) fed `SkillServiceConfig.FileSkills`, which `RegisterSkillCommands` turned into real slash commands (`internal/chat/commands.go`'s `RegisterSkillCommand`), invokable from the chat UI (`ChatComposer.tsx`'s `action === "skill"` branch) and resolving to the skill's actual `Prompt` body sent to the agent. This path never covered *authored, non-builtin* content (file-based discovery was already dead per `phase-1/08`), so the redesign's motivation stands, but "never reached a model through any live mechanism" should read "reached a model only for the 8 now-cut builtins, through a mechanism this batch retires rather than reuses." `RegisterSkillCommands`/`SkillCommandRegistrar`, `internal/chat/commands.go`'s skill-command registration, and `ChatComposer.tsx`'s `action === "skill"` handling are now fully dead code (task `01` made `FileSkills` permanently `nil`, its only producer) — a follow-up to remove this scaffolding is logged in `TASKS/ESCALATIONS.md`'s 2026-08-21 entry for task `01`'s review, not yet filed as its own task.
