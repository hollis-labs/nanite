...
This began as the evidence artifact for audit-remediation task `10/02` and the
input to architect decision AD-13. It now describes the implementation after
the operator approved the four selective delegations and task `10/05`
completed them in the required order.

The reviewed pre-extraction inventory at commit `3942f3c4` was:
...
The implemented inventory is now 27 transport fields and 50 production
`SelfToolsTransport` receiver methods. `CallTool` remains the sole static
dispatch switch with the same 68 names in 66 clauses. Four concrete
collaborators own the approved cohesive behavior:

- `MessagingTools`: 9 receiver methods.
- `WorkTrackingTools`: 10 receiver methods plus the package-level shared
  `resolveProjectIDFromSession` policy.
- `AgentProfileTools`: 5 receiver methods.
- `PresentationTools`: 7 receiver methods, including the sole
  `resolvePanelAccess` trust gate.
...
| `AP` | `self_tools_agent_profiles.go` |
...
| `M` | `self_tools_messaging.go` |
| `WT` | `self_tools_work_tracking.go` |
...
| Agent profile management and source resolution | `agent_create`, `agent_list`, `agent_update`, `agent_source_resolve` | `AgentProfileTools` owns a five-operation `AgentProfileStore` and `AgentClassifier` | `callCreateAgent`, `callListAgents`, `callUpdateAgent` (`T`); `classifyAgent` (`AP`); `callAgentSourceResolve` (`AS`) | `store`, `internal/agent` management classification, MCP | Implemented selective delegation. One owner holds the entity operations, classifier fallback, source resolution, and the single editability gate. The broad transport `Store` remains for unapproved domains. |
...
| Capability | Tool names | Fields read / reached | Receiver methods (file) | External dependencies | Shared state and extractability |
|---|---|---|---|---|---|
| Todo and plan work tracking | `todo_create`, `todo_update`, `todo_list`, `plan_create`, `plan_update`, `plan_step_add`, `plan_list`, `plan_get`, `plan_delete` | `WorkTrackingTools` owns an 11-operation `WorkTrackingStore`, `SessionProjectLookup`, and `WorkBroadcaster` | Nine handlers (`T`); `notifyWorkChanged` and package-level `resolveProjectIDFromSession` (`WT`) | `store` todo/plan models, MCP session identity, envelope marker output, work-change broadcaster | Implemented selective delegation. Every successful mutation broadcasts once and reads do not broadcast. Source correction: todo, reminder, and pin share the one session-to-project resolver; plans have no `project_id` and retain their legacy non-workspace missing-scope behavior of using `sessionID` as `scope_id`. |
| Internal messaging and session handoff | `message_send`, `message_inbox`, `message_thread`, `message_ack`, `message_resolve`, `message_catch_up`, `handoff_request`, `handoff_approve`, `handoff_reject` | `MessagingTools` owns `*messaging.Service` and `mcp.ElicitationService` | All nine handlers (`M`) | `internal/messaging`, MCP elicitation and caller/session context | Implemented selective delegation. The owner carries the common timeout, directive-elicitation policy, context-derived session identity, and nil-safe unwired behavior. Glass-4 handoff remains separate. |
...
| Reminder and pinned-context controls | `reminder_set`, `context_pin`, `context_unpin` | `Store` R/W, `ReminderEngine` W | `callSetReminder`, `callPin`, `callUnpin`, `currentTurnCount` (`RP`); shares package-level `resolveProjectIDFromSession` (`WT`) | `store`, `internal/reminders`, MCP session identity | Remains on the transport by design. Reminder and pin project autofill reuse the exact same resolver as todo; no separate implementation was introduced. |
...
| Per-turn scratchpad gate | `scratchpad_write`, `scratchpad_read`, `scratchpad_clear` | None | Inline branch in `CallTool` (`T`) | Chat-loop `loopState` owns the real implementation | Do not create an owner here. These names are cataloged self-tools but deliberately unavailable through this transport path; this branch only explains that boundary to CLI-launch callers. |
...
| Card presentation | `card_show` | `PresentationTools` owns `PanelLookup` and `PanelTrustResolver` | `callShowCard`, `resolveShowCardRenderTarget` (`T`); uses the owner's shared `resolvePanelAccess` (`P`) | `envelope` schema validation, turn source-ID validation, MCP context, panel trust | Implemented as one combined presentation owner. Card target resolution and panel operations use the same security decision point. |
| Panel and mode signals | `panel_open`, `panel_close`, `signal_mode` | `PresentationTools` owns `PanelLookup`, `PanelTrustResolver`, and `PanelSignalSink` (`PS`) | `callPanelOpen`, `callPanelClose`, `callSignalMode`, `resolvePanelAccess`, `emitPanelSignal` (`P`) | `dispatch` trust tiers, MCP caller/session identity, frontend stream sink | Implemented with card presentation. Built-ins stay nil-safe, plugin panels fail closed without trusted resolution, and signal emission retains the existing same-turn/API behavior. |
...
|---|---|---|
| `Store` | Skills, subagent recovery/recursion, task/reflex dispatch, chat history, procedures, reminders/pins, Glass-4 handoff, task-update reactions, scheduling | It remains the broad transport dependency for unapproved domains. Agent profiles and work tracking now receive narrow interfaces; this task deliberately did not begin a blanket Store split. |
...
| `PresentationTools.resolvePanelAccess` plus `PanelLookup`/`PanelTrustResolver` | Panel open/close and `card_show` render targets | One combined owner now preserves the security-relevant decision point exactly once. |
...
| Package-level `resolveProjectIDFromSession` | Todo creation/listing and reminder/pin scoping | One narrow `SessionProjectLookup` policy is shared without making either domain own the other. Plans do not use it because their persisted model has no `project_id`. |
...
the concrete services behind `Background`, `MessagingTools`,
`WorkTrackingTools`, `ReminderEngine`, and `Reactions` each own their state.
That
supports selective delegation, not a uniform one-type-per-tool-domain rewrite.

## Implemented inventory reconciliation

The 27 transport fields reconcile as follows:

- Shared persistence: `Store` (1).
- Approved collaborators: `MessagingTools`, `WorkTrackingTools`,
  `AgentProfileTools`, `PresentationTools` (4).
...
The transport's 50 receiver methods are the 82-method reviewed baseline minus
the 32 approved methods/policies moved off the receiver: messaging 9; work
tracking 11 (9 handlers, broadcast helper, and the resolver moved to a
package-level seam); agent profiles 5; and presentation 7. The four owners
hold 31 receiver methods in total because the shared project resolver is now a
...
## Implemented AD-13 direction

AD-13 adopted **selective delegation**, not a uniform “one owner per domain”
split.
`SelfToolsTransport` remains the MCP catalog/dispatch adapter and routes
...
Task `10/05` implemented the approved candidates in this serial order:
...
No fifth extraction is authorized by this map. The remaining capability rows
retain their original judgments and must be assessed through separately scoped
work rather than extending `10/05`.

## Coverage observations

Task `10/05` added characterization and routing coverage for the approved
messaging, work, agent-profile, and presentation boundaries. The original
observations for unapproved domains remain:
