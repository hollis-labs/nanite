...
For a new core type, update and release go-envelopes first, then bump Nanite's module pin. Nanite's generators invoke the released `github.com/hollis-labs/go-envelopes/cmd/envelopes-export` command through `scripts/lib/envelope-catalog.mjs`; they do not locate module-cache files or require a sibling checkout. Run `make generate-envelopes` (`scripts/generate-envelope-types.mjs`) for `ui/src/generated/envelope-types.generated.ts`. Add the matching renderer under `ui/src/components/chat/envelopes/`, recording the normal `component`, `export`, and `props` mapping in the upstream manifest. Use `CORE_OVERRIDES` in `scripts/generate-plugin-imports.mjs` only for an intentional Nanite-only deviation. Run `npm run generate:plugins` from `ui/` to refresh `ui/src/generated/plugin-envelopes.ts`, then run `make check-envelopes`. Never hand-edit generated registries. Verify both the streaming SSE path and persisted reload path.
