...
// planbridge.go — C2's PlanFromLaunch adoption for the standalone
// launcher (S5 platform-reshape).
//
// # What changed
//
// The launcher previously hand-assembled a shared agentlaunch.LaunchPlan
// via bootprofile.LaunchSpec.ToLaunchPlan (the retired hand-rolled
// bridge). Phase C flips it to the shipped agentlaunch.PlanFromLaunch
// seam:
//
...
//     registry has no binding or is unreachable (D1 + §4.1);
//   - agent identity is resolved CALLER-SIDE (§4.2) — it is never a
//     LaunchSpec or directory entity;
//   - the LaunchPlan is assembled by agentlaunch.PlanFromLaunch, the
//     pure shipped bridge, NOT a hand-rolled builder.
//
...
//
// Standalone launches run the long-lived NDJSON-over-stdio shape Nanite
// registers for claude (the same RuntimeStreamingStdio the previous
// planOptionsFor baked in). codex/opencode still spawn subprocess-per-
// turn inside Nanite's runtime; the plan-level runtime kind only has to
// be matrix-legal, and streaming-stdio is.
...
// a LaunchSpec/directory entity). The standalone launcher derives it
// from the compiled profile's identity block.
...
// buildLaunchPlan assembles a validated shared agentlaunch.LaunchPlan
// from a compiled bootprofile.LaunchSpec via agentlaunch.PlanFromLaunch.
//
// reg is the shared registrar; when nil the resolution is fully
// file/spec-default (the launcher still works — D1, offline-safe). The
...
func buildLaunchPlan(spec *bootprofile.LaunchSpec, reg *agentregistry.Registry, log *slog.Logger) (agentlaunch.LaunchPlan, error) {
	if spec == nil {
		return agentlaunch.LaunchPlan{}, fmt.Errorf("launcher: buildLaunchPlan: nil spec")
	}
	if log == nil {
		log = slog.Default()
	}
	...
	// The standalone launcher's runner id is the bare provider name the
	// compiler normalized onto spec.Provider.
	...
	if reg != nil {
		...
		if err != nil {
			return agentlaunch.LaunchPlan{}, fmt.Errorf("launcher: resolve runtime binding: %w", err)
		}
	} else {
		// No registry wired — the launch is fully file/spec-default. This
		// is the offline standalone path; the fallback is authoritative.
		if verr := fallback.Validate(); verr != nil {
			return agentlaunch.LaunchPlan{}, fmt.Errorf("launcher: file/spec runtime binding invalid: %w", verr)
		}
		runtime = fallback
	}
	...
	render := agentlaunch.RenderResult{
		...
		ResolvedInputs: map[string]any{
			...
			// the standalone launcher reserves a long-lived workspace dir
			// under WorkspacesRoot (agent.Boot does the actual
			// reservation; the plan is built + validated, never planted).
			// This matches the WorkspacePersistent the retired
			// planOptionsFor baked in.
	...
	if err != nil {
		return agentlaunch.LaunchPlan{}, fmt.Errorf("launcher: assemble launch plan: %w", err)
	}
