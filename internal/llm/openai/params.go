...
func buildChatParams(req llmtypes.ChatRequest) (sdk.ChatCompletionNewParams, error) {
	...
	if tools := translateTools(req.Tools); len(tools) > 0 {
		params.Tools = tools
		// Keep the existing non-reasoning Chat Completions path only on
		// models that actually accept none. Other reasoning turns use Responses.
		if modelSupportsReasoningNone(req.Model) {
...
func assistantMessages(m llmtypes.ChatMessage) ([]sdk.ChatCompletionMessageParamUnion, error) {
	...
	for _, b := range m.ContentBlocks {
		...
		case "tool_use":
			...
			toolCalls = append(toolCalls, sdk.ChatCompletionMessageToolCallUnionParam{OfFunction: &sdk.ChatCompletionMessageFunctionToolCallParam{
				ID: b.ID,
