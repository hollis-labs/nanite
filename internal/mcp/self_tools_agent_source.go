...
func agentSourceResolveToolDefinition() Tool {
	return Tool{
...
func (st *SelfToolsTransport) callAgentSourceResolve(args map[string]any) (*ToolResult, error) {
	if st == nil || st.Store == nil {
		return errorResult("agent_source_resolve: store not available"), nil
	}
	ref := strings.TrimSpace(strArg(args, "agent", ""))
	if ref == "" {
		return errorResult("agent_source_resolve: `agent` (id or slug) is required"), nil
	}

	agent := resolveAgentByRef(st.Store, ref)
	if agent == nil {
		return errorResult(fmt.Sprintf("agent_source_resolve: no agent profile found for %q", ref)), nil
	}
	...
	if err != nil {
		return errorResult(fmt.Sprintf("agent_source_resolve: marshal result: %v", err)), nil
	}
