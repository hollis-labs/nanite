...
func resolveChatTarget(st *SelfToolsTransport, raw string) (*store.Session, *ToolResult) {
	raw = strings.TrimSpace(raw)
	if raw == "" {
		return nil, errorResult("chat target is required (short_code like c248, or session_id UUID)")
	}
	if st == nil || st.Store == nil {
		return nil, errorResult("internal: store unavailable")
	}
	...
	if isShortCode(candidate) {
		...
		if err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return nil, errorResult(fmt.Sprintf("no chat found with short_code %q (try the chat list — short codes look like c248)", raw))
			}
			return nil, errorResult(fmt.Sprintf("resolve short_code %q: %v", raw, err))
		}
		return sess, nil
	}
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, errorResult(fmt.Sprintf("no chat found with session_id %q", raw))
		}
		return nil, errorResult(fmt.Sprintf("resolve session_id %q: %v", raw, err))
	}
	return sess, nil
}
...
func (st *SelfToolsTransport) callChatSearch(ctx context.Context, args map[string]any) (*ToolResult, error) {
	...
	if targetArg != "" {
		...
		if sessionID == "" {
			return errorResult("no session_id in context — chat_search requires a session context, or pass `target` (short_code/session_id) to search a specific chat"), nil
		}
	}
	...
	if len(snippets) == 0 {
		...
		return textResult(fmt.Sprintf("No matches found for %q in %s (scope=%s).", query, where, scope)), nil
	}
...
func (st *SelfToolsTransport) callChatGet(ctx context.Context, args map[string]any) (*ToolResult, error) {
	...
	if targetArg == "" {
		return errorResult("target is required (short_code like c248, or session_id UUID)"), nil
	}
	...
	if err != nil {
		return errorResult(fmt.Sprintf("list messages for %s: %v", sess.ShortCode, err)), nil
	}
	...
	out, err := json.Marshal(payload)
	if err != nil {
		return errorResult(fmt.Sprintf("marshal chat_get result: %v", err)), nil
	}
	return textResult(string(out)), nil
}
