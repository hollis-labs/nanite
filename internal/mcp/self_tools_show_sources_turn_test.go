...
func TestCallShowCard_Sources_AcceptsTurnToolUseIDs(t *testing.T) {
	st := newSelfTools(t)
	ctx := WithTurnToolUseIDs(context.Background(), []string{"toolu_real"})
	args := map[string]any{
		"type":    "report-card",
...
func TestCallShowCard_Sources_RejectsMixedRealAndFakeIDs(t *testing.T) {
	st := newSelfTools(t)
	ctx := WithTurnToolUseIDs(context.Background(), []string{"toolu_a", "toolu_b"})
	args := map[string]any{
		"type": "report-card",
		"data": cloneMap(validShowCardPayloads["report-card"]),
		"sources": `[
...
func TestTurnToolUseIDsFromContext_NilWithoutStamp(t *testing.T) {
	if got := TurnToolUseIDsFromContext(context.Background()); got != nil {
		t.Errorf("bare ctx must return nil, got %v", got)
	}
	if got := TurnToolUseIDsFromContext(nil); got != nil {
		t.Errorf("nil ctx must return nil, got %v", got)
	}
	// Empty input → ctx unchanged → still nil out.
	ctx := WithTurnToolUseIDs(context.Background(), nil)
	if got := TurnToolUseIDsFromContext(ctx); got != nil {
		t.Errorf("WithTurnToolUseIDs(nil) must leave ctx un-stamped, got %v", got)
	}
	ctx = WithTurnToolUseIDs(context.Background(), []string{})
	if got := TurnToolUseIDsFromContext(ctx); got != nil {
...
func TestTurnToolUseIDsFromContext_DefensiveCopy(t *testing.T) {
	src := []string{"toolu_a"}
	ctx := WithTurnToolUseIDs(context.Background(), src)
	src[0] = "tampered"
	got := TurnToolUseIDsFromContext(ctx)
