// Regression test for BLG-20260412-009 — adapter-codex atomic writes.
...
// AGENTS.md (partial or otherwise) must appear.
func TestPopulateSandbox_NoPartialFileOnWriteFailure(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, "AGENTS.md")); !os.IsNotExist(err) {
		t.Fatalf("AGENTS.md present after failed write: %v", err)
	}
