...
func (p *SupportPlugin) Load(host plugin.Host) error {
	...
	// Wire ticket store to Conduit's SQLite DB for persistence across restarts.
	if svc, err := host.GetService("store"); err == nil {
		...
		if s, ok := svc.(hasDB); ok {
			...
			if st, ok := svc.(*conduitstore.Store); ok {
				p.store.SetDB(st.DB)
				logger.Info("ticket store backed by SQLite")
			}
		}
...
// User-created data (tickets) is preserved.
func (p *SupportPlugin) Uninstall(host plugin.Host) error {
	...
	if err := db.DeleteAgent("it-support"); err != nil {
		return fmt.Errorf("remove IT Support agent: %w", err)
	}
	host.Logger().Info("uninstalled support plugin — removed IT Support agent profile")
	return nil
}
