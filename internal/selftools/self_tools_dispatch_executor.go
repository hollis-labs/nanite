...
func dispatchExecutorToolDefinition() mcp.Tool {
	return mcp.Tool{
...
func (st *SelfToolsTransport) callDispatchExecutor(ctx context.Context, args map[string]any) (*mcp.ToolResult, error) {
	if st.Executor == nil {
		return mcp.ErrorResult("dispatch_executor: executor is not configured (envelope_render not wired)"), nil
	}

	intent := strings.TrimSpace(strArg(args, "intent", ""))
	userRequest := strings.TrimSpace(strArg(args, "user_request", ""))
	if intent == "" {
		return mcp.ErrorResult("dispatch_executor: intent is required"), nil
	}
	if userRequest == "" {
		return mcp.ErrorResult("dispatch_executor: user_request is required (pass the user's verbatim ask)"), nil
	}
	...
	if srcErr != nil {
		return mcp.ErrorResult(fmt.Sprintf("dispatch_executor: %v", srcErr)), nil
	}
	if len(sources) > 0 {
		req.Sources = sources
	}
	...
	if err != nil {
		...
		// dropped dispatch.
		return mcp.ErrorResult(fmt.Sprintf("dispatch_executor: %v", err)), nil
	}
	if resp == nil {
		...
		return mcp.ErrorResult("dispatch_executor: nil response from executor (wiring regression)"), nil
	}

	body, err := json.Marshal(resp)
	if err != nil {
		return mcp.ErrorResult(fmt.Sprintf("dispatch_executor: marshal response: %v", err)), nil
	}
