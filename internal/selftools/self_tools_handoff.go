...
func handoffStashToolDefinition() mcp.Tool {
	return mcp.Tool{
		Name: "handoff_stash",
...
func handoffPointersExpandToolDefinition() mcp.Tool {
	return mcp.Tool{
...
func (st *SelfToolsTransport) callHandoffStash(ctx context.Context, args map[string]any) (*mcp.ToolResult, error) {
	...
	if err != nil {
		return mcp.ErrorResult(fmt.Sprintf("handoff_stash: marshal: %v", err)), nil
	}
...
func (st *SelfToolsTransport) callHandoffPointersExpand(ctx context.Context, args map[string]any) (*mcp.ToolResult, error) {
	...
	if payload, perr := ctxpkg.ParseHandoffEnvelope([]byte(row.Payload)); perr == nil {
		...
		if mErr != nil {
			return mcp.ErrorResult(fmt.Sprintf("handoff_pointers_expand: marshal: %v", mErr)), nil
		}
		return mcp.TextResult(string(body)), nil
	}
	return mcp.TextResult(row.Payload), nil
}
