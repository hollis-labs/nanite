...
func rememberToolDefinition() mcp.Tool {
	return mcp.Tool{
		Name: "lesson_capture",
...
// undiagnosed.
func (st *SelfToolsTransport) callRemember(ctx context.Context, args map[string]any) (*mcp.ToolResult, error) {
	if st.LearningRecorder == nil {
		return mcp.ErrorResult("lesson_capture: LearningRecorder is not configured (memory service unavailable)"), nil
	}

	scopeStr := strArg(args, "scope", "")
	scope := learnings.Scope(scopeStr)
	if !scope.IsValid() {
		return mcp.ErrorResult(fmt.Sprintf("lesson_capture: invalid scope %q (want tool_use|project|session)", scopeStr)), nil
	}
	subject := strings.TrimSpace(strArg(args, "subject", ""))
	if subject == "" {
		return mcp.ErrorResult("lesson_capture: subject is required (tool name / project_id / session_id)"), nil
	}
	hint := strings.TrimSpace(strArg(args, "hint", ""))
	if hint == "" {
		return mcp.ErrorResult("lesson_capture: hint is required and must be a non-empty string"), nil
	}
	...
	if err != nil {
		return mcp.ErrorResult(fmt.Sprintf("lesson_capture: %v", err)), nil
	}
	...
	if err != nil {
		...
		return mcp.ErrorResult(fmt.Sprintf("lesson_capture: marshal result: %v", err)), nil
	}
