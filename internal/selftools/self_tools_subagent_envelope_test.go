...
func TestCallSpawnSubagent_RecursionCap_RejectsParentedCaller(t *testing.T) {
	...
	rootCtx := mcp.WithSessionID(context.Background(), "sess-root")
	...
	childCtx := mcp.WithSessionID(context.Background(), "sess-child")
...
// subagent tests.
func parseEnvelopeFromResult(t *testing.T, res *mcp.ToolResult) subagent.ResultEnvelope {
	t.Helper()
	if res == nil {
...
func TestCallSpawnSubagent_ParentAgentIDFromCallerProfile(t *testing.T) {
	st := newSubagentTestTransport(t, subagent.EchoRunner{})
	ctx := mcp.WithCallerProfile(context.Background(), "agent-from-ctx")
