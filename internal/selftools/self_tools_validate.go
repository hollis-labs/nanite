...
func (st *SelfToolsTransport) callValidate(_ context.Context, args map[string]any) (*mcp.ToolResult, error) {
...
func validationResult(valid bool, errs []envelope.StructuredError) *mcp.ToolResult {
	if errs == nil {
	...
	if err != nil {
		...
		return mcp.ErrorResult(fmt.Sprintf("validate: marshal result: %v", err))
	}
