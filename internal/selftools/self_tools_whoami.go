...
func whoamiToolDefinition() mcp.Tool {
	return mcp.Tool{
		Name: "whoami",
...
func (st *SelfToolsTransport) executeWhoami(ctx context.Context, args map[string]any) (*mcp.ToolResult, error) {
	agentID := mcp.CallerProfileFromContext(ctx)
	if agentID == "" {
		return mcp.ErrorResult("whoami: no agent profile in context (not called from a durable-agent session)"), nil
	}
