...
func TestAssembleSlots_S3b_SelectionHashRebuild(t *testing.T) {
	...
	// First turn: 3 tools.
	first, err := svc.AssembleSlots(context.Background(), sess, agent, nil, tools3(), "", 200000, "")
	if err != nil {
	...
	reduced := tools3()[:2]
	second, err := svc.AssembleSlots(context.Background(), sess, agent, nil, reduced, "", 200000, "")
	if err != nil {
...
func TestAssembleSlots_S3b_StateTransitionTracking(t *testing.T) {
	...
	first, _ := svc.AssembleSlots(context.Background(), sess, agent, nil, tools3(), "", 200000, "")
	...
	second, _ := svc.AssembleSlots(context.Background(), sess, agent, nil, tools3(), "", 200000, "")
...
func TestAssembleSlots_S3b_TokensBeforeReflectsPriorSlot(t *testing.T) {
	...
	second, err := svc.AssembleSlots(context.Background(), sess, agent, nil, tools3(), "", 200000, "")
	if err != nil {
	...
	third, err := svc.AssembleSlots(context.Background(), sess, agent, nil, tools3(), "", 200000, "")
	if err != nil {
