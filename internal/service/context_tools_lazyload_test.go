...
func TestAssembleSlots_LazyHintAppendedToToolsSlot(t *testing.T) {
	...
	r, err := svc.AssembleSlots(context.Background(), sess, agent, nil, tools3(), "", 200000, hint)
	if err != nil {
...
func TestAssembleSlots_EmptyLazyHintIsNoOp(t *testing.T) {
	...
	// "" as the baseline).
	r, err := svc.AssembleSlots(context.Background(), sess, agent, nil, tools3(), "", 200000, "")
	if err != nil {
		t.Fatalf("AssembleSlots: %v", err)
	}
	...
	// identical content.
	r2, err := svc.AssembleSlots(context.Background(), sess, agent, nil, tools3(), "", 200000, "")
	if err != nil {
...
func TestAssembleSlots_LazyHintInvalidatesCacheKey(t *testing.T) {
	...
	// Turn 1: no hint.
	r1, err := svc.AssembleSlots(context.Background(), sess, agent, nil, tools3(), "", 200000, "")
	if err != nil {
	...
	r2, err := svc.AssembleSlots(context.Background(), sess, agent, nil, tools3(), "", 200000, "[lazy hint v1]")
	if err != nil {
	...
	r3, err := svc.AssembleSlots(context.Background(), sess, agent, nil, tools3(), "", 200000, "[lazy hint v1]")
	if err != nil {
	...
	r4, err := svc.AssembleSlots(context.Background(), sess, agent, nil, tools3(), "", 200000, "[lazy hint v2]")
	if err != nil {
