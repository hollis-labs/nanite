	...
	return []DurableAgentRecipe{
		{
			...
			Inputs: []DurableAgentRecipeInput{
				...
				recipeModelInput(false, "claude-sonnet-4"),
				...
				recipeStringInput("project_scope", "Project scope", "metadata.project_scope", true, "agridd"),
		...
		},
		{
			...
			Description:      "A durable managed CLI harness session for local tool-backed work controlled through Agridd chat and harness APIs.",
		...
		},
		{
			...
			Inputs: []DurableAgentRecipeInput{
				...
				recipeRuntimeKindInput(false, string(runtimekind.API)),
				recipePathInput("work_root", "Work root", "durable_agent.work_root", false, "~/dev/project"),
				recipeStringInput("scope_topic", "Scope / topic", "metadata.scope_topic", true, "agridd frontend/runtime API"),
		...
		},
		{
			...
			Kind:             DurableAgentRecipeKindProjectAdvisor,
			Name:             "Proxima Relay",
			Description:      "An operator-facing relay/concierge durable agent that can front other durable agents through Agridd surfaces.",
			LifecycleClass:   store.DurableAgentClassAdvisor,
			ProfileRule:      "operator_selected",
			Provider:         "anthropic",
			Model:            "claude-sonnet-4",
		...
		},
		{
			...
			Inputs: []DurableAgentRecipeInput{
				...
				recipeStringInput("system_name", "System / service name", "metadata.system_name", true, "agridd api"),
		...
		},
		{
			...
			Kind:             DurableAgentRecipeKindProjectAdvisor,
			Name:             "Web Chat Agent",
			Description:      "A durable advisor prepared to back a web chat experience through Agridd sessions and the harness v1 API.",
			LifecycleClass:   store.DurableAgentClassAdvisor,
			ProfileRule:      "operator_selected",
			Provider:         "anthropic",
			Model:            "claude-sonnet-4",
