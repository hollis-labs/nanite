...
// migration 110's SQL backfill: a newly created row with no explicit
...
// validation (mirroring the DB-level CHECK migration 110 adds) rejects any
...
// before migration 110) is rejected going forward.
func TestValidateAgentMultiAgentFields_ActivationModeThreeValueEnum(t *testing.T) {
	...
	if err := validateAgentMultiAgentFields(a); err == nil {
		t.Error("activation_mode 'instance': expected rejection, got nil (it was retired by migration 110)")
	}
}
