...
// TestMigration057_Idempotency covers the PR #114 review concerns
...
//
// This test re-applies the 057 statement directly against rows we craft,
// independent of goose's real ledger (which now guarantees this migration
// runs at most once against any given database). The statement's own
// WHERE-guard idempotency still matters in its own right — e.g. a manual
// re-run via `goose redo`, or copy-pasting the pattern into a future
// migration — so this test probes the SQL directly rather than through
// s.migrate().
func TestMigration057_Idempotency(t *testing.T) {
	dbPath := t.TempDir() + "/migration057test.db"
	...
	// observe migration 057's behavior end-to-end.
	...
	// Load the migration 057 SQL from the embedded FS so the test exercises
	// the real statement, not a copy.
	mig, err := migrationsFS.ReadFile("migrations/057_chat_harness_executor_handoff_capability.sql")
	if err != nil {
	...
	if update057 == "" {
		t.Fatalf("no UPDATE prompt_templates statement found in migration 057")
	}

	// First run: should rewrite the row with the new bullet and bump updated_at.
	res, err := s.DB.Exec(update057)
	if err != nil {
