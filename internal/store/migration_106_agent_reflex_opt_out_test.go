...
// TestMigrate106AddsOptOutColumnAndTable is the regression test for
...
func TestMigrate106AddsOptOutColumnAndTable(t *testing.T) {
	s := newTestStore(t)
	...
	// non-Go-layer inserts: pre-migration rows backfilled by 106's own Up,
	...
	if _, err := s.DB.ExecContext(ctx,
		`INSERT INTO agent_reflexes
		 ...
		 VALUES ('rfx-migration-106-probe', NULL, 'process', 'migration-106-probe',
	...
	seedWorkspace(t, s, "ws-migration-106")
	...
	if err := s.migrate(); err != nil {
		t.Fatalf("re-migrate after 106 already applied: %v", err)
	}
}

// TestMigrate106AgentDeleteCascadesOptOuts verifies the other cascade
...
func TestMigrate106AgentDeleteCascadesOptOuts(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()
	...
	seedWorkspace(t, s, "ws-migration-106-agent-delete")
...
// TestMigrate106DownDropsOptOutColumnAndTable is the tested Down half of
// migration 106.
func TestMigrate106DownDropsOptOutColumnAndTable(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()
	...
	// Insert must still work against the pre-106 shape (no opt_out_allowed
	// column to satisfy). Uses a raw INSERT rather than InsertAgentReflex —
	// InsertAgentReflex is current Go code, which always binds
	// opt_out_allowed and therefore assumes the current (post-106) schema;
