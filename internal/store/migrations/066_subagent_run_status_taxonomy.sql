...
-- (2026-era) column set. Goose's real ledger (see
-- docs/engineering/architecture/05-storage-and-migrations.md) tracks this
-- migration as applied exactly once, ever, so the retired
-- "migrate:skip-if-column-exists" directive that used to guard this rebuild
-- against re-running on every boot is no longer needed.
--
