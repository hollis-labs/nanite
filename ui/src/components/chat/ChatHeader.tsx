...
export function ChatHeader() {
  ...
  const contextChip = ctxCeiling > 0
  ...
  const switchAgentMutation = useMutation({
    mutationFn: async (agentId: string) => {
      ...
      if (primaryAgent) {
        try { await api.removeSessionAgent(activeSessionId, primaryAgent.agent_id) } catch { /* ignore */ }
      }
                ...
                <div className="absolute left-0 top-full z-50 mt-1 w-56 rounded-[10px] border border-border-subtle bg-bg-elevated py-1 shadow-2xl">
                  ...
                  {allAgents.filter((a) => a.status !== 'disabled').map((agent) => (
                    <button
                      key={agent.id}
                      type="button"
                      onClick={() => { switchAgentMutation.mutate(agent.id); setAgentDropOpen(false) }}
            ...
            <div className="absolute right-0 top-full z-50 mt-1 w-52 rounded-[10px] border border-border-subtle bg-bg-elevated py-1 shadow-2xl">
              {agentCount > 1 && (
                <>
                  <button
                    type="button"
                    onClick={() => { setRosterOpen(true); setMoreOpen(false) }}
