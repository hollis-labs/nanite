...
export function TriviaLeaderboardCard({ data, onSendMessage }: TriviaLeaderboardCardProps) {
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 ease-out">
      <div className="rounded-lg border border-amber-500/20 bg-zinc-900/50 overflow-hidden max-w-sm">
        {/* Header */}
        <div className="flex items-center gap-2 px-4 pt-3 pb-2 border-b border-zinc-800">
          <Trophy className="h-4 w-4 text-amber-400 shrink-0" />
          <span className="text-sm font-semibold text-zinc-200">Trivia Leaderboard</span>
        </div>
        ...
        <div className="px-4 py-2 border-t border-zinc-800 flex items-center justify-between">
          <span className="text-[10px] text-zinc-600 uppercase tracking-wider">Trivia Game</span>
          {onSendMessage && (
            <button
              type="button"
