  ...
  render: () => {
    ...
    return {
      onStart: (props: SuggestionProps<SlashCommand>) => {
        ...
        // Use flushSync so the ref callback fires synchronously before control
        // returns to TipTap. Without this, menuRef is null when onKeyDown fires
        // immediately after the menu opens (e.g. user presses Enter right away),
        // causing key events to fall through and the selection to be missed.
    ...
    function renderMenu(r: Root, props: SuggestionProps<SlashCommand>) {
      // flushSync forces React to commit the render (including ref callbacks)
      // synchronously. This guarantees menuRef is populated before onKeyDown
      // can fire, preventing the "first keypress after opening the menu is
      // ignored" reliability bug across consecutive slash command uses.
      flushSync(() => {
        r.render(
          <SlashCommandMenu
            items={props.items}
