...
export function MemoryModal() {
  ...
  const handleClose = useCallback((isOpen: boolean) => {
    if (!isOpen) {
      setOpen(false)
      setTimeout(() => { setView('browse'); setSelectedKey(null) }, 200)
    }
  }, [setOpen])
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl h-[600px] p-0 flex flex-col overflow-hidden">
        {view === 'browse' && (
          <MemoryBrowse
            onSelect={(key) => { setSelectedKey(key); setView('detail') }}
            onCreate={() => { setSelectedKey(null); setView('create') }}
          />
        )}
        {(view === 'detail' || view === 'create') && (
          <MemoryDetail
            memoryKey={view === 'create' ? null : selectedKey}
            onBack={() => { setView('browse'); setSelectedKey(null) }}
          />
        )}
      </DialogContent>
    </Dialog>
