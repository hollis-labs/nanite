...
export function PromptTemplateEditor({}: PromptTemplateEditorProps) {
  ...
  const getScopeTone = (scope: string): StatusTone => {
    switch (scope) {
      case "system":  return "info";
      case "mode":    return "success";
              ...
              return (
                <ContextMenu key={template.id}>
                  <ContextMenuTrigger asChild>
                    <div
                      className="rounded-xl border border-border-subtle bg-bg-elevated overflow-hidden transition-all cursor-pointer hover:border-border border-l-2 group"
                      style={{ borderLeftColor: template.is_builtin ? 'var(--color-status-ok)' : 'var(--color-brand)' }}
                      onClick={() => setSelectedTemplate(template.id)}
                    >
                      {/* Header */}
                      <div className="flex items-center gap-2.5 px-3.5 py-3">
                        <span className="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-surface text-fg-secondary group-hover:text-fg transition-colors shrink-0">
                          <DynamicIcon
                            name={template.icon}
                            className="w-4 h-4"
                            fallback={FileText}
                      ...
                      {/* Detail footer */}
                      <div className="border-t border-border-subtle px-3.5 py-2 bg-bg/40 flex items-center gap-2">
                        <StatusPill tone={getScopeTone(template.scope)}>
                          {template.scope}
                        </StatusPill>
