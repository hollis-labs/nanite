...
export function RecentExecutionsTable({ data }: RecentExecutionsTableProps) {
  ...
  if (data.length === 0) {
    return <p className="text-xs text-fg-faint italic py-2">No executions recorded yet</p>
  }
        ...
        <tbody>
          {sorted.map((row) => (
            <tr key={row.id} className="border-b border-border/30 hover:bg-surface/20 transition-colors">
              ...
              <td className="py-1.5 pr-3 text-right font-mono tabular-nums text-fg-secondary">{formatTokens(row.input_tokens)}</td>
