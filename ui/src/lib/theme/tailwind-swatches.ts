...
// Family → shade → hex
const TW_PALETTE: Record<string, Record<number, string>> = {
  slate: {
...
export const TW_FAMILIES = Object.keys(TW_PALETTE)
export const TW_SHADES = [50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 950] as const

export function getSwatch(family: string, shade: number): string | undefined {
  return TW_PALETTE[family]?.[shade]
}
...
export function allSwatches(): Swatch[] {
  ...
  for (const family of TW_FAMILIES) {
    for (const shade of TW_SHADES) {
      const hex = TW_PALETTE[family]?.[shade]
      if (hex) out.push({ family, shade, hex, name: `${family}-${shade}` })
    }
  }
  return out
}

/** Find a Tailwind family/shade label from a hex value (exact match only). */
export function matchSwatch(hex: string): { family: string; shade: number } | undefined {
  const lower = hex.toLowerCase()
  for (const family of TW_FAMILIES) {
    for (const shade of TW_SHADES) {
      if (TW_PALETTE[family]?.[shade]?.toLowerCase() === lower) {
        return { family, shade }
      }
    }
  }
